...
for (i in 1: length(unique(colocate$chromosome))) {
  ...
  trait.plot<-plot_grid(Wet,Dry,logdiff,align="v",nrow=3,rel_heights=trait.count+2.5)
  dendro.plot<-plot_grid(Wet_dendro,Dry_dendro,logdiff_dendro,align="v",nrow=3,rel_heights=trait.count+2.5)
