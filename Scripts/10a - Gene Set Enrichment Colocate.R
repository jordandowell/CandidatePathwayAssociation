...
if(length(geneNames)==length(mySignificantGenes)){
 ...
 #subset only significant genes for each trait
 ...
 for (j in 1:length(GO_term_files)) {
    ...
    try({
       ...
       #trim geneID2GO to just significant genes
       geneID2GO<-geneID2GO[1:as.numeric(count(GOfactor,pvalue)[1,2])]
       ...
       for (i in 1:length(GOTYPES)) {
         ...
         try( allRes <- GenTable(
          ...
          pdf(paste(
             "Plots/SingleTrait/GO/GO_SingleTrait_Significant_",GO_resultnames[j],"_",
             GOTYPES[i],
             ".pdf",
             sep = ""
          ))
